...
It will be helpful to listen to the lectures before going through this notebook since the concepts of the model and approach are discussed in the lecture.
...
In this course, they talk about several cloud GPU options and provided a conda environment fastai in those environments. I am using the Paperspace setup that is covered in [Lesson 1](http://course.fast.ai/lessons/lesson1.html). Reshama Shaik has also listed detailed upto date [instructions](https://github.com/reshamas/fastai_deeplearn_part1/blob/master/tools/paperspace.md) on getting set up in Paperspace. 
...
For some reason, the epoch is not printing fully (but it is calculated) in the notebook in RStudio. But the html output is very verbose. For now, I am not showing it in output.
